...
void Ayuda(void);
void Guardar_bitacora(int Error, char *dato);


int num_datos=0;
char cad_aux[10],dato;
int i;
int main(int argc, char *argv[])
{	
	system("clear");
	int posicion=atoi(argv[3]);
	/*for (i = 0; i < argc; ++i)
	{
	...
	switch (*argv[2]){
		...
		case 'e':
			...
			if(posicion>num_datos){
				printf("%d",posicion );
				printf("%d",num_datos );
				printf("--> Error: La Posicion no es valida ó no se encuentra \n");
				Guardar_bitacora(4,argv[3]);
				return 0;
			}
...
void Guardar_bitacora(int Error, char *dato){
    ...
    fprintf(bitacora, "El tipo de error fue:%s\n" );
    switch(Error){
    	case 1:
